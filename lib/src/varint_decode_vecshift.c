 ...
 *
 * data: uint8_t pointer to the start of the compressed varints
...
size_t varint_decode_vecshift(const uint8_t *data, size_t length, uint32_t *output)
{
    ...
    while (length > 0)
    {
        ...
        if (number_of_varints == vl)
        {
            ...
            __riscv_vse32_v_u32m4(output, __riscv_vzext_vf4_u32m4(input, vl), vl);
            data += vl;
            length -= vl;
            output += vl;
            processed += vl;
        ...
        else
        {
            ...
            vuint8m1_t v1 = __riscv_vslide1down_vx_u8m1(input, 0, vl);
            ...
            vuint8m1_t first_bytes = __riscv_vcompress_vm_u8m1(input, m_first_bytes, vl);
